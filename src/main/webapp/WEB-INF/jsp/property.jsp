...
<head>
    <title>Properties</title> <!-- CHANGE THIS TO THE TEXT THAT SHOULD APPEAR IN THE TAB BAR -->
    <meta charset="utf-8">
...
<div class="content">
    ...
    <body>
 <h2>Properties</h2> <!-- THIS SHOULD BE TITLE OF YOUR PAGE-->
...
<table>
    ...
    <c:forEach var="item" items="${propertyList}">
        <tr id="tableData">
            <td>${home_builderRepo.findById(item.getHome_Builder_ID()).get().getHome_Builder()}</td>
            <td>${property_styleRepo.findById(item.getProperty_Style_ID()).get().getStyle()}</td>
            <td>${item.getStreet_Address()}</td>
            <td>${item.getCity()}</td>
            <td>${stateRepo.findById(item.getState_ID()).get().getState_Name()}</td>
            <td>${item.getZip_Code()}</td>
            <td>${countryRepo.findById(item.getCountry_ID()).get().getCountry_Name()}</td>
            <td>
                <form action="/PropertyEdit/${item.getProperty_ID()}">
            ...
            <td>
                <form action="/PropertyDelete/${item.getProperty_ID()}">
