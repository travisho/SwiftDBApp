...
<html>
    ...
    <head>
        <title>Edit Commission Plan</title>
        <meta charset="utf-8">
...
<c:forEach var="item" items="${commission_planRepo.findAll()}">
    <c:if test="${CommissionPlanID == item.getCommission_Plan_ID()}">

        <h2>Edit Commission Plan</h2>
        ...
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/Commission_PlanEdited">
                    <td><input type="text" name="totalCom" value="${item.getTotal_Commission()}"></td>
                    <td><input type="text" name="YunCom" value="${item.getYun_Global_Commission_Percent()}"></td>
                     <!-- Broker Name Dropdown Start -->
                    <td>
                        <select name="broker">
                            <option value="${item.getBroker_ID()}">
                                    ${brokerRepo.findById(item.getBroker_ID()).get().getBroker_Name()}
                            </option>
                    ...
                    <!-- Broker Name Dropdown End -->
                    <td><input type="text" name="brokerCom" value="${item.getBroker_Commission_Percent()}"></td>
                          <td>
                             <select name="contract">
                                 <option value="${item.getContract_ID()}"/>
                                     ${propertyRepo.findById(contractRepo.findById(item.getContract_ID()).get().getProperty_ID()).get().getStreet_Address()}
                                 ...
                                 <c:forEach var="itemb" items="${contractRepo.findAll()}">
                                     <option value="${itemb.getContract_ID()}">
                                         ${propertyRepo.findById(itemb.getProperty_ID()).get().getStreet_Address()} for ${clientRepo.findById(itemb.getClient_ID()).get().getClient_First_Name()} ${clientRepo.findById(itemb.getClient_ID()).get().getClient_Last_Name()}
                                     </option>
                                 </c:forEach>
                             <select>
                         </td>
                    <td><input type="hidden" name="id" value="${item.getCommission_Plan_ID()}"><input type="submit" value="Submit"></td>
                </form>
            </tr>
