...
<head>
    <title>Homeowner Associates</title>
    <meta charset="utf-8">
...
<table>
    <tr>
        ...
        <th>HOA Last Name</th>
        <th>Homeowner Company</th>
        <th colspan="2">Actions</th>
    </tr>


    <tr class="filterBG">
        <form action="/Homeowner_AssociateFilter">

            <td><input type="text" name="hoafname" value=""></td>
            <td><input type="text" name="hoalname" value=""></td>
            <td><input type="text" name="hoacomp" value=""></td>
            <td><button class="btn"><i class="fa fa-search" style="color:white;"></i></button></td>
        </form>
        <td>
            <form action="/Homeowner_AssociateRedirect">
            <button>New HOA</button>
            </form>
        </td>
    </tr>


    <c:forEach var="item" items="${homeowner_associateList}">
        <tr id="tableData">

            <!--td>${propertyRepo.findById(item.getProperty_ID()).get().getStreet_Address()}</td !-->
            <td>${item.getHOA_First_Name()}</td>
            <td>${item.getHOA_Last_Name()}</td>
            <td>${item.getHOA_Company()}</td>

            <td>
                <form action="/Homeowner_AssociateEdit/${item.getHOA_ID()}">
                    <input type="submit" value="Edit">
                </form>
            </td>

            <td>
                <form action="/Homeowner_AssociateDelete/${item.getHOA_ID()}">
