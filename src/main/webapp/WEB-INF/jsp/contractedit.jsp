...
<head>
    <title>Edit Contract</title> <!-- CHANGE THIS TO THE TEXT THAT SHOULD APPEAR IN THE TAB BAR -->
    <meta charset="utf-8">
...
<c:forEach var="item" items="${contractRepo.findAll()}">
    <c:if test="${ContractID == item.getContract_ID()}">

        <h2>Edit Contract Information for ${clientRepo.findById(item.getClient_ID()).get().getClient_First_Name()}
        ${clientRepo.findById(item.getClient_ID()).get().getClient_Last_Name()} at
        ${propertyRepo.findById(item.getProperty_ID()).get().getStreet_Address()}</h2>
        ...
        <tr>
            ...
            <tr class="filterBG">
                <form method = "put" action="/ContractEdited">
                    <td>
                        <select name="client">
                    ...
                    <td>
                        <select name="agent">
                            ...
                            <c:forEach var="itemb" items="${agentRepo.findAll()}">
                                <option value="${itemb.getAgent_ID()}">
                                    ${itemb.getAgent_First_Name()}
                                    ${itemb.getAgent_Last_Name()}
                                </option>
                            </c:forEach>
                        </select>
                    </td>
                    ...
                    <td>
                        <select name="street">
                            ...
                            <c:forEach var="itemc" items="${propertyRepo.findAll()}">
                                <option value="${itemc.getProperty_ID()}">
                                    ${itemc.getStreet_Address()}
                                </option>
                            </c:forEach>
                        </select>
                    </td>
                    ...
                    <td>
                        <select name="status">
                            <option value="${item.getContract_Status_ID()}">
                                ${contract_statusRepo.findById(item.getContract_Status_ID()).get().getContract_Status()}
                            </option>
                            <c:forEach var="itemd" items="${contract_statusRepo.findAll()}">
                                <option value="${itemd.getContract_Status_ID()}">
                                    ${itemd.getContract_Status()}
                                </option>
                            </c:forEach>
                        </select>
                    </td>
                    ...
                    <td>
                        <select name="type">
                            <option value="${item.getContract_Type_ID()}">
                                ${contract_typeRepo.findById(item.getContract_Type_ID()).get().getContract_Type()}
                            </option>
                            <c:forEach var="iteme" items="${contract_typeRepo.findAll()}">
                                <option value="${iteme.getContract_Type_ID()}">
                                    ${iteme.getContract_Type()}
                                </option>
                            </c:forEach>
                        </select>
                    </td>

                    <td><input type="date" name="movein" value="${item.getMove_In_Date()}"></td>
                    <td><input type="text" name="city" value="${item.getDesired_City()}"></td>
                    <td><input type="hidden" name="id" value="${item.getContract_ID()}">
