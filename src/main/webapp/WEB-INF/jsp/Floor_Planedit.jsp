...
<head>
    <title>Editing Floor Plan</title>
    <meta charset="utf-8">
...
<c:forEach var="item" items="${Floor_PlanRepo.findAll()}">
    <c:if test="${Floor_Plan_ID == item.getFloor_Plan_ID()}">

        <h2>Editing Floor Plan Information for Floor ${item.getFloor_Number()} at ${PropertyRepo.findById(item.getProperty_ID()).get().getStreet_Address()}</h2>
        ...
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/Floor_PlanEdited">
                    ...
                    <td><input type="text" name="Floor_Plan" value="${item.getFloor_Plan()}">
                    <td><input type="text" name="Floor_Number" value="${item.getFloor_Number()}">


                    <td><input type="hidden" name="id" value="${item.getFloor_Plan_ID()}">
