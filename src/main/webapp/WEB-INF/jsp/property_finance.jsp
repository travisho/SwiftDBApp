...
<head>
    <title>Property Finances</title> <!-- CHANGE THIS TO THE TEXT THAT SHOULD APPEAR IN THE TAB BAR -->
    <meta charset="utf-8">
...
<table>
    <tr>
     ...
     <tr class="filterBG">
        <form action="/Property_FinanceFilter">
            <td><input type="text" name="street"></td>
            <td><input type="text" name="tax"></td>
            <td><input type="text" name="totalpe"></td>
            <td><input type="text" name="monthlype"></td>
            <td><button class="btn"><i class="fa fa-search" style="color:white;"></i></button></td>
        </form>

        <td>
        <form action="/Property_FinanceRedirect">
            <button>New Property Finance</button>
        </form>
        </td>

    </tr>

    <c:forEach var="item" items="${property_financeList}">
        <tr id="tableData">

            <td>${propertyRepo.findById(item.getProperty_ID()).get().getStreet_Address()}</td>
            <td>${item.getProperty_Tax()}</td>
            <td>${item.getTotal_Price_Estimate()}</td>
            <td>${item.getMonthly_Price_Estimate()}</td>

            <td>
                <form action="/Property_FinanceEdit/${item.getProperty_Finance_ID()}">
            ...
            <td>
                <form action="/Property_FinanceDelete/${item.getProperty_Finance_ID()}">
