...
<table>
    <tr>
        <th>Floor Plan</th>
        <th>Bathtub Width</th>
        <th>Bathtub Length</th>
        <th>Bathroom Width</th>
        <th>Bathroom Length</th>
        <th colspan="2">Actions</th>
    </tr>

    <tr class="filterBG">
        <form action="/BathroomFilter">
            <td><input type="text" name="Floor_Plan" value=""></td>
            <td><input type="text" name="Bathtub_Width" value=""></td>
            <td><input type="text" name="Bathtub_Length" value=""></td>
            <td><input type="text" name="Bathroom_Width" value=""></td>
            <td><input type="text" name="Bathroom_Length" value=""></td>
            <td><button class="btn"><i class="fa fa-search" style="color:white;"></i></button></td>
             </form>
        <td>
            <form action="/BathroomRedirect">
                <button>New Bathroom</button>
            </form>
        </td>
         </tr>

         <c:forEach var="item" items="${BathroomList}">
             <tr id="tableData">
            <td>Floor ${Floor_PlanRepo.findById(item.getFloor_Plan_ID()).get().getFloor_Number()} at ${PropertyRepo.findById(Floor_PlanRepo.findById(item.getFloor_Plan_ID()).get().getProperty_ID()).get().getStreet_Address()}</td>
            <td>${item.getBathtub_Width()}</td>
            <td>${item.getBathtub_Length()}</td>
            <td>${item.getBathroom_Width()}</td>
            <td>${item.getBathroom_Length()}</td>

            <td>
                <form action="/BathroomEdit/${item.getBathroom_ID()}">
                    <button type="submit" value="Edit">
                        <i class="fas fa-edit"></i>
                </form>
            </td>
            <td>
                <form action="/BathroomDelete/${item.getBathroom_ID()}">
