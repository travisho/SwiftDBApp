...
<head>
    <title>Editing Agent Email</title>
    <meta charset="utf-8">
...
<c:forEach var="item" items="${Agent_EmailRepo.findAll()}">
    <c:if test="${Agent_Email_ID == item.getAgent_Email_ID()}">


    <h2>Editing Agent Email Information for ${AgentRepo.findById(item.getAgent_ID()).get().getAgent_First_Name()} ${AgentRepo.findById(item.getAgent_ID()).get().getAgent_Last_Name()}</h2>
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/Agent_EmailEdited">
                    <td>
                        <select name="agent">
                    ...
                    <td><input type="text" name="Agent_Email" value="${item.getAgent_Email()}">

                    <td><input type="hidden" name="id" value="${item.getAgent_Email_ID()}">
