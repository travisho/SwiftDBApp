...
<head>
    <title>Clients Phone Edit</title>
    <meta charset="utf-8">
...
<body>
<c:forEach var="item" items="${client_Phone_NumberRepo.findAll()}">
    <c:if test="${Client_Phone_Number_ID == item.getClient_Phone_Number_ID()}">


<h2>Client Phone Number edit</h2>
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/ClientPhoneEdited">
                    <td>
                    <select name="client">
                        ...
                        <option value="${itema.getClient_ID()}">
                                ${clientRepo.findById(itema.getClient_ID()).get().getClient_First_Name()}
                                ${clientRepo.findById(itema.getClient_ID()).get().getClient_Last_Name()}
                        </option>
                        </c:forEach>
                    </select>
                    </td>
                    <td><input type="text" name="phone" value="${item.getClient_Phone_Number()}"></td>
                    <td><input type="text" name="type" value="${item.getClient_Type()}"></td>

                    <td><input type="hidden" name="id" value="${item.getClient_Phone_Number_ID()}"><input type="submit" value="Submit"></td>
                </form>
            </tr>
