...
<c:forEach var="item" items="${discussionRepo.findAll()}">
    <c:if test="${DiscussionID == item.getDiscussion_ID()}">
        ...
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/DiscussionEdited">
                <!-- Agent Dropdown Start -->
                   <td>
                       <select name="agent">
                           <option value="${item.getAgent_ID()}"/>
                               ${agentRepo.findById(item.getAgent_ID()).get().getAgent_First_Name()} ${agentRepo.findById(item.getAgent_ID()).get().getAgent_Last_Name()}
                 ...
                 <!-- Agent Dropdown End -->
                                ...
                                <td>
                                    <select name="client">
                                        <option value="${item.getClient_ID()}"/>
                                            ${clientRepo.findById(item.getClient_ID()).get().getClient_First_Name()} ${clientRepo.findById(item.getClient_ID()).get().getClient_Last_Name()}

                                        <c:forEach var="itemb" items="${clientRepo.findAll()}">
                                            <option value="${itemb.getClient_ID()}">
                                                ${itemb.getClient_First_Name()}
                                                ${itemb.getClient_Last_Name()}
                                            </option>
                                        </c:forEach>
                                    </select>
                                </td>
                                <!-- Client Style Dropdown End -->

                                <!-- Medium Dropdown Start -->
                                <td>
                                    <select name="medium">
                                         <option value="${item.getDiscussion_Medium_ID()}"/>
                                                ${discussion_mediumRepo.findById(item.getDiscussion_Medium_ID()).get().getDiscussion_Medium()}

                                         <c:forEach var="itemc" items="${discussion_mediumRepo.findAll()}">
                                            <option value="${itemc.getDiscussion_Medium_ID()}">
                                            ${itemc.getDiscussion_Medium()}
                                            </option>
                                         </c:forEach>
                                    </select>
                                </td>
                    ...
                    <td><input type="text" name="time" value="${item.getDiscussion_Time()}"></td>
                    <td><input type="text" name="topic" value="${item.getDiscussion_Topic()}"></td>
                    <td><input type="text" name="summary" value="${item.getDiscussion_Summary()}"></td>
                    <td><input type="hidden" name="id" value="${item.getDiscussion_ID()}"><input type="submit" value="Submit"></td>
                </form>
            </tr>
