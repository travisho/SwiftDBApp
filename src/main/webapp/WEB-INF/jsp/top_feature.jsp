...
<table>
    ...
    <tr class="filterBG">
        <form action="/TopFeatureFilter">
            <td></td>
            <td></td>
            <td></td>
            <td><input type="text" name="top_feature"></td>
            <td><button class="btn"><i class="fa fa-search" style="color:white;"></i></button></td>
        </form>

        <td>
            <form action="/TopFeatureRedirect">
                <button>New Top Feature</button>
            </form>
        </td>
    </tr>
    <c:forEach var="item" items="${topfeatureList}">
        <tr id="tableData">
            <td>${propertyRepo.findById(item.getProperty_ID()).get().getStreet_Address()}</td>
            <td>${propertyRepo.findById(item.getProperty_ID()).get().getCity()}</td>
            <td>${propertyRepo.findById(item.getProperty_ID()).get().getZip_Code()}</td>
            <td>${item.getTop_Feature()}</td>
            <td>
                <form action="/TopFeatureEdit/${item.getTop_Feature_ID()}">
                    <button type="submit" value="Edit">
                        <i class="fas fa-edit"></i></button>
                </form>
            </td>
            <td>
                <form action="/TopFeatureDelete/${item.getTop_Feature_ID()}">
