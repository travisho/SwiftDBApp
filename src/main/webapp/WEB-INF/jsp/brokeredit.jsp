...
<c:forEach var="item" items="${BrokerRepo.findAll()}">
    <c:if test="${Broker_ID == item.getBroker_ID()}">
        <h2>Broker Edit</h2>
        ...
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/BrokerEdited">
                    <td><input type="text" name="name" value="${item.getBroker_Name()}"></td>
                    <!-- Company Dropdown Start -->
                    <td>
                        <select name="company">
                            <option value="${item.getBroker_Company_ID()}">
                                    ${Broker_CompanyRepo.findById(item.getBroker_Company_ID()).get().getBroker_Company()}
                            </option>
                            <c:forEach var="itemb" items="${Broker_CompanyRepo.findAll()}">
                                <option value="${itemb.getBroker_Company_ID()}">
                                        ${itemb.getBroker_Company()}
                                </option>
                            </c:forEach>
                        </select>
                    </td>
                    <!-- Company Dropdown End -->
                    <td><input type="hidden" name="id" value="${item.getBroker_ID()}"><input type="submit" value="Submit"></td>
                </form>
            </tr>
        </table>
