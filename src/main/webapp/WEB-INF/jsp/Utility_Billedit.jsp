...
<c:forEach var="item" items="${Utility_BillRepo.findAll()}">
    <c:if test="${Utility_Bill_ID == item.getUtility_Bill_ID()}">


        <h2>Edit Utility Bill Information for ${PropertyRepo.findById(Property_FinanceRepo.findById(item.getProperty_Finance_ID()).get().getProperty_ID()).get().getStreet_Address()}</h2>
        <table>

            <tr>
                <th>Street Address</th>
                <th>Utility Bill</th>
                <th>Cost of Utility</th>
                <th colspan="2">Actions</th>

            </tr>

            <tr class="filterBG">
                <form method = "put" action="/Utility_BillEdited">
                    <td>
                        <select name="Property">
                            <option value="${item.getProperty_Finance_ID()}">
                                ${PropertyRepo.findById(Property_FinanceRepo.findById(item.getProperty_Finance_ID()).get().getProperty_ID()).get().getStreet_Address()}
                            <c:forEach var="itema" items="${Property_FinanceRepo.findAll()}">
                                <option value="${itema.getProperty_Finance_ID()}">
                    ...
                    <td><input type="text" name="Utility" value="${item.getUtility()}"></td>

                    <td><input type="text" name="Cost_Of_Utility" value="${item.getCost_Of_Utility()}"></td>


                    <td><input type="hidden" name="id" value="${item.getUtility_Bill_ID()}">
