...
<head>
    <title>Edit Property</title> <!-- CHANGE THIS TO THE TEXT THAT SHOULD APPEAR IN THE TAB BAR -->
    <meta charset="utf-8">
...
<c:forEach var="item" items="${propertyRepo.findAll()}">
    <c:if test="${PropertyID == item.getProperty_ID()}">

        <h2>Edit ${item.getStreet_Address()}</h2>
        ...
        <tr>
            ...
            <tr class="filterBG">
                <form method = "put" action="/PropertyEdited">

                    <!-- Home Builder Dropdown Start -->
                    <td>
                        <select name="builder">
                            <option value="${item.getHome_Builder_ID()}">
                                ${home_builderRepo.findById(item.getHome_Builder_ID()).get().getHome_Builder()}
                            </option>
                    ...
                    <td>
                        <select name="style">
                            <option value="${item.getProperty_Style_ID()}">
                                ${property_styleRepo.findById(item.getProperty_Style_ID()).get().getStyle()}
                            </option>
                            <option value=""/>
                    ...
                    <td><input type="text" name="street" value="${item.getStreet_Address()}"></td>
                    <td><input type="text" name="city" value="${item.getCity()}">

                    <!-- State Dropdown Start -->
                    <td>
                        <select name="state">
                            <option value="${item.getState_ID()}">
                                ${stateRepo.findById(item.getState_ID()).get().getState_Name()}
                            </option>
                    ...
                    <td>
                        <select name="country">
                            <option value="${item.getCountry_ID()}">
                                ${countryRepo.findById(item.getCountry_ID()).get().getCountry_Name()}
                            </option>
                    ...
                    <td><input type="hidden" name="id" value="${item.getProperty_ID()}">
