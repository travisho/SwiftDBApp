...
<head>
    <title>Agents</title> <!-- CHANGE THIS TO THE TEXT THAT SHOULD APPEAR IN THE TAB BAR -->
    <meta charset="utf-8">
...
<table>
    ...
    <c:forEach var="item" items="${agentList}">
        <tr id="tableData">
            <td>${item.getAgent_First_Name()}</td>
            <td>${item.getAgent_Last_Name()}</td>
            <td>${item.getTitle()}</td>
            <td>
                <form action="/AgentEdit/${item.getAgent_ID()}">
            ...
            <td>
                <form action="/AgentDelete/${item.getAgent_ID()}">
