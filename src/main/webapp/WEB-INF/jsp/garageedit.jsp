...
<head>
    <title>Edit Garage Information</title>
    <meta charset="utf-8">
...
<c:forEach var="item" items="${garageRepo.findAll()}">
<tr id="tableData">
    <c:if test="${Garage_ID == item.getGarage_ID()}">
    ...
    <table>
        ...
        <tr class="filterBG">
            <form method = "put" action="/GarageEdited">

                <!-- Street Address Dropdown Start -->
                <td>
                    <select name="street">
                        <option value="${item.getFloor_Plan_ID()}">
                                ${propertyRepo.findById(floor_planRepo.findById(item.getFloor_Plan_ID()).get().getProperty_ID()).get().getStreet_Address()}
                        </option>
                        <c:forEach var="itema" items="${floor_planRepo.findAll()}">
                        <option value="${itema.getFloor_Plan_ID()}">
                                ${propertyRepo.findById(itema.getProperty_ID()).get().getStreet_Address()}                            </c:forEach>
                    </select>
                </td>
                <!-- Street Address Dropdown End -->

                <td><input type="text" name="width" value="${item.getGarage_Width_ft()}"></td>
                <td><input type="text" name="length" value="${item.getGarage_Length_ft()}"></td>
                <td><input type="text" name="capacity" value="${item.getGarage_Car_Capacity()}"></td>

                <td><input type="hidden" name="id" value="${item.getGarage_ID()}">
