...
<head>
    <title>Edit Agent Phone Number</title>
    <meta charset="utf-8">
...
<div class="content">
    ...
    <body>
        ...
        <c:forEach var="item" items="${Agent_PhoneRepo.findAll()}">
            <c:if test="${Agent_PhoneID == item.getAgent_Phone_Number_ID()}">

        <h2>Edit Agent ${agentRepo.findById(item.getAgent_ID()).get().getAgent_First_Name()}
                ${agentRepo.findById(item.getAgent_ID()).get().getAgent_Last_Name()} Phone Number</h2>
                ...
                <table>
                    ...
                    <tr class="filterBG">
                        <form method = "put" action="/AgentPhoneEdited">
                            <td>
                            <select name="agent">
                                ...
                                <option value="${itema.getAgent_ID()}">
                                        ${AgentRepo.findById(itema.getAgent_ID()).get().getAgent_First_Name()}
                                        ${AgentRepo.findById(itema.getAgent_ID()).get().getAgent_Last_Name()}
                                </option>
                                </c:forEach>
                            </select>
                            </td>
                            <td><input type="text" name="phone" value="${item.getAgent_Phone_Number()}"></td>
                            <td><input type="text" name="type" value="${item.getAgent_Phone_Type()}"></td>
                            <td><input type="hidden" name="id" value="${item.getAgent_Phone_Number_ID()}"><input type="submit" value="Submit"></td>
                        </form>
                    </tr>
                </table>
                <!-- Table ends here -->
