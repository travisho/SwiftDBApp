...
<table>
    ...
    <tr>
        ...
        <th>Garage Width (ft.)</th>
        <th>Garage Length (ft.)</th>
        <th>Garage Car Capacity</th>
        <th colspan="2">Actions</th>
    </tr>
    </thead>

    <tr class="filterBG">
        <form action="/GarageFilter">
       ...
       <td>
           <form action="/GarageRedirect">
            <button>New Garage</button>
           </form>
       </td>
    </tr>

    <c:forEach var="item" items="${garageList}">
        <tr id="tableData">

            <td>${propertyRepo.findById(floor_planRepo.findById(item.getFloor_Plan_ID()).get().getProperty_ID()).get().getStreet_Address()}</td>
            <td>${item.getGarage_Width_ft()}</td>
            <td>${item.getGarage_Length_ft()}</td>
            <td>${item.getGarage_Car_Capacity()}</td>

            <td>
                <form action="/GarageEdit/${item.getGarage_ID()}">
            ...
            <td>
                <form action="/GarageDelete/${item.getGarage_ID()}">
