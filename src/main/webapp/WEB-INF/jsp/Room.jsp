...
<h2>Room Information at Properties</h2>
...
<table>
    ...
    <form action="/RoomFilter">
        ...
        <td><input type="text" name="Room_Type" value="${item.getRoom_Type()}">
        <td><input type="text" name="Room_Area_Ft" value="${item.getRoom_Area_ft()}">
...
<%--            <td>${RoomRepo.findById(item.getRoom_ID()).get().getRoom_Type()}</td>--%>
    <td>${PropertyRepo.findById(Floor_PlanRepo.findById(item.getFloor_Plan_ID()).get().getProperty_ID()).get().getStreet_Address()}</td>
            <td>${item.getRoom_Type()}</td>
            <td>${item.getRoom_Area_ft()}</td>
            <td>
                <form action="/RoomEdit/${item.getRoom_ID()}">
                    <button type="submit" value="Edit">
                        <i class="fas fa-edit"></i>
                </form>
            </td>
            <td>
                <form action="RoomDelete/${item.getRoom_ID()}">
