...
<head>
    <title>Editing Agent</title> <!-- CHANGE THIS TO THE TEXT THAT SHOULD APPEAR IN THE TAB BAR -->
    <meta charset="utf-8">
...
<c:forEach var="item" items="${agentList}">
    <c:if test="${AgentID == item.getAgent_ID()}">

        <h2>Edit Agent ${item.getAgent_First_Name()} ${item.getAgent_Last_Name()}</h2>
        ...
        <table>
            ...
            <tr class="filterBG">
                <form method = "put" action="/AgentEdited">
                    <td><input type="text" name="first" value="${item.getAgent_First_Name()}"></td>
                    <td><input type="text" name="last" value="${item.getAgent_Last_Name()}"></td>
                    <td><input type="text" name="title" value="${item.getTitle()}"></td>
                    <td><input type="hidden" name="id" value="${item.getAgent_ID()}">
